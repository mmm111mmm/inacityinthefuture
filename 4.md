...
## We can only assign a value with the correct type

If we declare a string, we can *only* assign a string value to it.

If we decare a float, we can *only* assign a float value to it.

Otherwise Java will complain. A lot.
