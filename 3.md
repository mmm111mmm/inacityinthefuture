...
We cannot concatenate a int with an int, for example.

We cannot concatenate a int with a float. 

We can only concatenate a string with something else.
...
*But* if we have a string somewhere, it will work:
